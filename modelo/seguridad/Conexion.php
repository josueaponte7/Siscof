...
class Conexion
{
    ...
    protected function execute_query($sql, $tip)
    {
    ...
    protected function select($options, $type = 'BOTH')
    {
        switch ($type) {
            case 'ASSOC':
                $type = MYSQLI_ASSOC;
    ...
    protected function row($options)
    {
        ...
        if ($this->_state_conn && $this->_state_query && $this->_state_query->num_rows > 0) {
            ...
            $this->_state_query->free();
            $data_response['datos'] = $this->_res;
            return $data_response;
        }else{
            $data_response['datos'] = 0;
            return $data_response;
        }
    ...
    protected function numRows($table = NULL, $field = NULL, $conditions = '1')
    {
        if ($table === NULL || $field === NULL) {
            ...
            $this->_state_query = $this->_conn->query($sql);
            if ($this->_state_conn && $this->_state_query && $this->_state_query->num_rows > 0) {
                $resultado['num_rows'] = $this->_state_query->num_rows;
                $this->_state_query->free();
            }
            $this->_close_conn();
            return $resultado;
    ...
    public function autoIncrement($table = NULL, $field = 'id')
    {
        if ($table === NULL || $field === NULL) {
            return FALSE;
        } else {
    ...
    protected function insert($table = NULL, $array_of_values = array())
    {
        if ($table === NULL || empty($array_of_values) || !is_array($array_of_values)) {
            ...
            if ($this->_state_conn) {
                ...
                /*$this->_sql = "SELECT $this->_id FROM $table ORDER BY 1 DESC LIMIT 1";
                ...
                if ($resul === 0) {
                    ...
                    $resultado['id']               = $this->_conn->insert_id;
                    $resultado['success']          = 'ok';
                    $resultado['columns_affected'] = $this->_conn->affected_rows;
                    $this->_rows_affected = $this->_conn->affected_rows;
                } else {
                    $resultado = $resul;
                }
